...
def shipping_to_checkout_dict(shipping: AddressData) -> Dict:
    if shipping.phone:
        import phonenumbers
        phoneNumber = phonenumbers.parse(shipping.phone)
    return {
        "phone": {
            "country_code":phoneNumber.country_code if shipping.phone else "",
            "number": phonenumbers.format_number(phoneNumber, phonenumbers.PhoneNumberFormat.INTERNATIONAL) if shipping.phone else ""
        },
        "address": {
