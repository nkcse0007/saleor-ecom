...
def authorize(
    ...
    stripe_amount = get_amount_for_checkout(payment_information.amount, currency)
    ...
    try:
        ...
        success = payment_res.status in ("Authorized")
        response = _success_response(
            intent=payment_res, kind=kind, success=success, customer_id=payment_res.customer.id
        )
...
def capture(payment_information: PaymentData, config: GatewayConfig) -> GatewayResponse:
    ...
    intent = None
    try:
        intent = client.payments.capture(payment_information.payment_id, amount=payment_information.amount, reference='CAPTURE')
    except sdk.errors.CheckoutSdkError as exc:
        action_required = intent.status == "request_invalid" if intent else False
